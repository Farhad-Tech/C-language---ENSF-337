...
void String_truncate(String *dest, int new_length){
    ...
    char* new_ptr = malloc ((new_length+1) * sizeof(char));
    int oldLength= strlen((*dest).dynamic_storage);
    if(new_length<oldLength) {
      for (int i=0; i<new_length ; i++) {
        new_ptr[i]= (*dest).dynamic_storage[i];
      }
      new_ptr[new_length]='\0';
      dest->dynamic_storage=new_ptr; //leaked memory will return to pool automatically
