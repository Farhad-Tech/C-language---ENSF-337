...
void String_append(String *dest, const String* source){
    ...
    String temp = {NULL, 0};
    temp.dynamic_storage = calloc(((dest->length)+(source->length)+1), sizeof(char));
    for (int i=0; i<dest->length; i++) {
        temp.dynamic_storage[i] = dest->dynamic_storage[i];
    }
    temp.length = strlen(temp.dynamic_storage);
    for (int i=0; i<=source->length; i++) {
        temp.dynamic_storage[i+temp.length] = source->dynamic_storage[i];
    }
    temp.length = strlen(temp.dynamic_storage);
    dest->dynamic_storage = temp.dynamic_storage;
    dest->length = temp.length;
...
void String_truncate(String *dest, int new_length){
    ...
    char* new_point = malloc((new_length+1) * sizeof(char));
    int old_length = strlen((*dest).dynamic_storage);
    if(new_length<old_length) {
      for (int i=0; i<new_length ; i++) {
        new_point[i] = (*dest).dynamic_storage[i];
      }
      new_point[new_length]='\0';
      dest->dynamic_storage=new_point; //leaked memory will return to pool automatically
...
void test_copying(void){
    ...
    //copies "William Shakespeare" in the string_storage in object st1
